...
interface LayoutStage {
  ...
  mission: string;
  hint: string;
  correctHtml: string; // HTMLの合格キーワード
  correctCss: string;  // CSSの合格キーワード
...
const STAGES: Record<ModeType, LayoutStage[]> = {
  site: [
    ...
    { 
      id: 5, 
      title: "5. メディア全体の3カラムタイル (CSS Grid)", 
  ...
  wordpress: [
    ...
    { 
      id: 5, 
      title: "5. 【最終試練】自作テーマの統合パーツ配置", mission: "HTMLで「single-article」を作り、中のアイキャッチと本文の隙間を30px空けて完全固定せよ！", 
      hint: "CSSで gap: 30px; を指定することで、WordPressの動的パーツ間に完璧な距離を作れます。", 
      correctHtml: "single-article",
      correctCss: "30px", 
...
export default function VisualLab() {
  ...
  // 状態データ
  const [visualStyles, setVisualStyles] = useState<Record<string, string>>({});
  ...
  useEffect(() => {
    ...
    stage.controls.forEach(c => initial[c.property] = c.options);
    setVisualStyles(initial);
  ...
  // HTMLとCSSの「両方」の合格をリアルタイム検知するセンサー
  ...
  const getContainerStyle = () => {
    if (step === 'visual') {
      return {
        ...
        ...visualStyles,
        width: '100%', height: '100%', minHeight: '280px', background: '#141414', padding: '20px', rounded: '8px'
      };
    } else {
      // 両方合格なら正解レイアウト、不合格なら初期状態
      return {
        display: stage.id >= 4 && mode === 'site' || stage.id >= 3 && mode === 'wordpress' && currentIdx >= 2 ? 'grid' : 'flex',
  ...
  return (
    <div className="space-y-4 text-left">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {/* 左側：段階に応じたコントローラー（設計パネルか、HTML/CSSの2タブエディタ） */}
                ...
                <div key={ctrl.property} className="space-y-1 text-xs">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {ctrl.options.map((opt) => (
                      <button key={opt} onClick={() => setVisualStyles(p => ({ ...p, [ctrl.property]: opt }))} className={`px-2 py-0.5 text-[10px] font-mono rounded border ${visualStyles[ctrl.property] === opt ? 'bg-[#0e639c] text-white border-cyan-500' : 'bg-[#2d2d2d] text-slate-400 border-transparent'}`}>{opt}</button>
                    ))}
                  </div>
                </div>
              ))}
